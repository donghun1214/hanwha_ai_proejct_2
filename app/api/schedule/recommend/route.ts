  ...
  properties: {
    ...
    items: {
      ...
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['taskId', 'startTime', 'endTime', 'decision', 'riskScore', 'reason'],
        properties: {
          taskId: { type: 'number' },
...
function isTask(value: unknown): value is TaskInput {
  ...
  return typeof task.id === 'number' && typeof task.name === 'string' &&
    ...
    Number.isFinite(task.duration) && Number.isFinite(task.requiredCrew) && Number.isFinite(task.priority);
}
...
export async function POST(request: Request) {
  ...
  try {
    const body = await request.json() as { tasks?: unknown; weatherSlots?: unknown; totalCrew?: unknown };
    ...
    if (!tasks.length || !weatherSlots.length || totalCrew < 1) {
    ...
    const result = JSON.parse(text) as {
      summary: string;
      items: Array<{ taskId: number; startTime: string; endTime: string; decision: string; riskScore: number; reason: string }>;
    };
